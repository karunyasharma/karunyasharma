...
<!-- Fill these in with your real certificates — placeholders below -->
- 🏅 *Add certification name* — Issuer, Year
- 🏅 *Add certification name* — Issuer, Year
- 🏅 *Add hackathon / competition result*

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:FFD400,100:050505&height=4&section=header" width="100%"/>

## 🧭 Learning Roadmap

- [x] Core ML & Data Analytics (Python, Pandas, scikit-learn)
- [x] Computer Vision fundamentals (OpenCV, TensorFlow)
- [ ] Deep dive into Generative AI & LLMs
- [ ] Production-grade MLOps (model deployment, monitoring)
- [ ] Advanced 3D web experiences (Three.js, React Three Fiber)
...
- 🧠 Computer vision pipelines — from data collection to real-time inference
- 📊 Turning messy raw datasets into clean, decision-ready dashboards
- 🎨 Building 3D, animation-heavy portfolio sites with Next.js + Three.js
- 🚀 How I structure student ML projects so they don't die after the deadline
...
[![Sponsor](https://img.shields.io/badge/SUPPORT-buy%20me%20a%20coffee-FFD400?style=for-the-badge&logo=buymeacoffee&logoColor=050505&labelColor=050505)](https://www.buymeacoffee.com/karunyasharma)
...
5. Isometric metrics dashboard: add metrics.yml to .github/workflows/ as well. It needs a Personal Access
   Token (repo + read:user scopes) saved as a repo secret named METRICS_TOKEN — the default GITHUB_TOKEN
   doesn't have enough permission for this one action. Create the PAT under Settings > Developer settings,
   then Settings > Secrets and variables > Actions > New repository secret in this repo.

6. The "Buy Me a Coffee" link uses a placeholder URL (buymeacoffee.com/karunyasharma) — swap it for your
   real page once you create one, or delete that badge if you don't want a support link.

7. Arcade contribution graph (pacman/bomberman): add pacman-contribution.yml to .github/workflows/ too.
   It pushes to an `output-arcade` branch (deliberately different from the snake's `output` branch) so
   the two actions don't overwrite each other's SVGs. No extra secrets needed — GITHUB_TOKEN is enough.
